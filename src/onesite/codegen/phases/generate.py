...
def _cleanup_retired_model_hook_modules(
    model: ModelDefinition, backend_path: Path
) -> None:
    """Remove modules generated by the retired ORM/background model hooks."""
    module_name = model["module_name"]
    (backend_path / "app" / "events" / f"{module_name}.py").unlink(
        missing_ok=True
    )
    (backend_path / "app" / "handlers" / f"{module_name}.py").unlink(
        missing_ok=True
    )
...
def phase_generate_per_model(
    ...
    for model in models:
        ...
        _cleanup_retired_model_hook_modules(model, backend_path)
